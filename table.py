...
#-------------------------------------------------------------------------------
class Courses:
    def __init__(self, id, name, instructors, maxNumbOfStudents):
        self._ID = id
        self._name = name
...
class Class:
    ...
    def __str__(self):
        return "{" + str(self._dept.get_name()) + "," + str(self._course.get_ID()) + "," + \
               str(self._room.get_number()) + "," + str(self._instructor.get_id()) + "," + str(
            self._TimeAvilable.get_id()) + "}"
#-------------------------------------------------------------------------------
class Data:
    ROOMS = [["R1", 25], ["R2", 45], ["R3", 35]]
    Time_Avilable = [["TA1", "MO 09:00 - 10:00"],
                     ["TA2", "MO 10:00 - 11:00"],
                     ["TA3", "Tu 09:00 - 10:30"],
                     ["TA4", "Tu 10:30 - 12:00"]]
    INSTRUCTORS = [["1", "DR Amr"],
                   ["2", "Mr Mohamed"],
                   ["3", "DR Hala"],
                   ["4", "Mrs Doaa"]]
    ...
    def __init__(self):
        ...
        course1 = Courses("C1", "DB", [self._instructors[0], self._instructors[1]], 25)
        course2 = Courses("C2", "DS", [self._instructors[0], self._instructors[1], self._instructors[2]], 35)
        course3 = Courses("C3", "IS", [self._instructors[0], self._instructors[1]], 25)
        course4 = Courses("C4", "logic", [self._instructors[2], self._instructors[3]], 30)
        course5 = Courses("C5", "math", [self._instructors[3]], 35)
        course6 = Courses("C6", "CO", [self._instructors[0], self._instructors[2]], 45)
        course7 = Courses("C7", "SA", [self._instructors[1], self._instructors[3]], 45)
        self._courses = [course1, course2, course3, course4, course5, course6, course7]
        dept1 = Department("IT", [course1, course3])
        dept2 = Department("AI", [course2, course4, course5])
        dept3 = Department("CS", [course6, course7])
...
class schedule:
    ...
    def get_classes(self):
        self.isFitnesschanged = True # true mean that i had calculate fit which mean fitnees is changed
        return self.classes
...
class genatic_algorithm:
    ...
    def crossover(self, popu):
        ##bick up two schedules to crossover them
        crossover_pop = Population(0) #initailize with empty population
...
class Display:
    ...
    def print_course(self):
        ...
        for i in range(0, len(courses)):
            ...
            availableCoursesTable.add_row(
                [courses[i].get_ID(), courses[i].get_name(), str(courses[i].get_maxNumbOfStudents()), tempStr])
    ...
    def print_schedule_as_table(self, schedule):
        ...
        table = prettytable.PrettyTable(
            ['Class #', 'Dept', 'Course(Id , max # of students)', 'Room (Capacity)', 'Instructor','TimeAvilable'])
        for i in range(0, len(classes)):
            table.add_row([str(i), classes[i].get_dept().get_name(), classes[i].get_course().get_name() + " (" +
                           classes[i].get_course().get_ID() + ", " + str(classes[i].get_course().get_maxNumbOfStudents()) + ")",
